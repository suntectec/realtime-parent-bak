...
public class FlinkUtils {
    ...
    public static StreamExecutionEnvironment getEnv(ParameterTool parameters) {
        ...
        //开启checkpoint
        env.enableCheckpointing(parameters.getLong("checkpoint.interval", 60000L));
        ...
        env.setRestartStrategy(RestartStrategies.fixedDelayRestart(parameters.getInt("checkpoint.restart.times", 3), Time.seconds(10)));
        /**
         * 设置要使用的状态后端
         */
        if (env instanceof LocalStreamEnvironment) {  // 在本地运行的逻辑
            ...
            env.getCheckpointConfig().setCheckpointStorage(parameters.get("checkpoint.state.backend.path","file:///C:\\Volumes\\flink"));
            log.info("flink提交作业模式：--本地");
        } else { // 在集群运行的逻辑
            EmbeddedRocksDBStateBackend embeddedRocksDBStateBackend = new EmbeddedRocksDBStateBackend(true);
            env.setStateBackend(embeddedRocksDBStateBackend);  // 设置 EmbeddedRocksDBStateBackend 作为状态后端
            env.getCheckpointConfig().setCheckpointStorage(parameters.get("checkpoint.state.backend.path","hdfs:///flink/flink-checkpoints"));
            log.info("flink提交作业模式：--集群");
        }
    ...
    public static Sink<String> getKafkaSink(String topic,String  kafkaServers) {
